...
from UNet_lstm import UNet16, UNetResNet
...
def create_model(device, type):
    if type == 'vgg16':
        print('create vgg16 model')
        model = UNet16(pretrained=True)
    elif type == 'pspnet_res34':
        num_classes = 1
        model = PSPNet(n_classes=num_classes, sizes=(1, 2, 3, 6), psp_size=512, deep_features_size=256, backend=34)
        print('create pspnet model')
    elif type == 'pspnet_res18':
        num_classes = 1
        model = PSPNet(n_classes=num_classes, sizes=(1, 2, 3, 6), psp_size=512, deep_features_size=256, backend=18)
        print('create pspnet model')
    elif type == 'resnet101':
        encoder_depth = 101
        num_classes = 1
        ...
        print('create SegNet model')
        model = SegNet(3, 1)
    # elif type == 'densenet57':
    #     print('create densenet model')
    #     model = FCDenseNet57(n_classes=1).cuda()
    else:
        assert False
    model.eval()
...
def train(train_loader, model, criterion, optimizer, scheduler, validation, args):
    ...
    for epoch in range(epoch, args.n_epoch + 1):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            # flood = (loss-0.05).abs()+0.05                        #de we need zero training loss after achieveing zero training error?
            losses.update(loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n_epoch', default=60, type=int, metavar='N', help='number of total epochs to run')
    ...
    parser.add_argument('-batch_size', default=1, type=int, help='weight decay (default: 1e-4)')
    parser.add_argument('-num_workers', default=4, type=int, help='output dataset directory')

    parser.add_argument('-data_dir', type=str, default='E:\\medical datasets\\Pancreas-CT\\train',
                        help='input dataset directory')
    parser.add_argument('-model_dir', type=str, default='E:\\medical datasets\\Pancreas-CT\\segnet',
                        help='output dataset directory')
    parser.add_argument('-model_type', type=str, required=False, default='segnet',
                        choices=['vgg16', ' resnet101', 'resnet34', 'resnet18', 'resnet50', 'segnet', 'densenet57', 'pspnet_res34', 'pspnet_res18'])
    ...
    img_names = [path.name for path in Path(DIR_IMG).glob('*.png')]
    ...
    train_loader = DataLoader(train_dataset, args.batch_size, drop_last=True, shuffle=False, pin_memory=torch.cuda.is_available(),
                              num_workers=args.num_workers)
    valid_loader = DataLoader(valid_dataset, args.batch_size, drop_last=True, shuffle=False, pin_memory=torch.cuda.is_available(),
                              num_workers=args.num_workers)

    model.cuda()

    train(train_loader, model, criterion, optimizer, scheduler, validate, args)
