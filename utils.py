...
import torch
import tqdm
from UNet import UNet16, UNetResNet
from SegNet import SegNet
from pspnet_lstm import PSPNet
...
def load_pspnet_res34(model_path):
    num_classes = 1
...
def load_pspnet_res18(model_path):
    num_classes = 1
...
def load_unet_resnet_18(model_path):
    model = UNetResNet(pretrained=True, encoder_depth=18, num_classes=1)
    checkpoint = torch.load(model_path)
    if 'model' in checkpoint:
        model.load_state_dict(checkpoint['model'])
    elif 'state_dict' in checkpoint:
...
def load_unet_resnet_50(model_path):
    model = UNetResNet(pretrained=True, encoder_depth=50, num_classes=1)
    checkpoint = torch.load(model_path)
    if 'model' in checkpoint:
        model.load_state_dict(checkpoint['model'])
    elif 'state_dict' in checkpoint:
